...
private:
  ...
  uint32_t m_port;
  uint32_t m_sourceNode[5];
  uint32_t m_sinkNode[5];              // number of sink nodes (< all nodes)
...
m_packetLoss (0),
m_numHops(0),
m_traceFile("/home/hry-user/mobility.tcl"),
...
/*  
	// 移動ノード
	mn = new Ptr<Node> [m_nNodes];
	for (uint32_t i = 0; i < m_nNodes; ++i){
		mn[i] = CreateObject<Node> ();
		//mn[i]->AggregateObject(CreateObject<ConstantPositionMobilityModel> ());
		  wlanNodes.Add(mn[i]);
	}
  m_adhocTxNodes.Add(wlanNodes);
  * 
  */
}
//メソッド15
...
VanetRoutingExperiment::ConfigureDevices ()//チャネルを構成する
{
  ...
  VanetRoutingExperiment::ConfigureApplications ()//アプリケーションを設定する
  {
    ...
    //m_routingHelper->Install(wlanNodes,movedevice,m_adhocTxInterfaces,m_totalSimTime,m_protocolName);
