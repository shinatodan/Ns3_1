...
private:
  ...
  std::string m_protocolName;       // seconds
  uint32_t m_port;
  uint32_t m_sourceNode[6];
  uint32_t m_sinkNode[6];              // number of sink nodes (< all nodes)
...
{//ノード上でルーティング・プロトコルを設定する c:ノードコンテナ
  ...
  else if(m_protocolName=="NGPSR"){
    list.Add (ngpsr, 100);//dsdvルーティングヘルパーとその優先度(100)を格納する
    internet.SetRoutingHelper (list);//インストール時に使用するルーティングヘルパーを設定する
    internet.Install(c);//各ノードに(Ipv4,Ipv6,Udp,Tcp)クラスの実装を集約する
    GetRoutingStats().SetCport(ngpsr::RoutingProtocol::NGPSR_PORT);
  }
...
{//ネットデバイスとそのインターフェイスにIPv4アドレスを割り当てる。
  //d:ネットデバイスコンテナ,adhoctxinterface:IPv4インターフェイスコンテナ
  ...
  //ネットワークアドレス:10.1.0.0,ネットワークマスク:255.255.0.0 割り当てられるIPアドレス10.1.0.1~10.1.255.254
  ic = addressAdhoc.Assign (d);//上記の設定に基づいてネットデバイスにIPアドレスを割り当てる
...
m_nNodes (36),//ノード数を156で初期化
m_protocolName ("OLSR"),//プロトコル名を"protocol"で初期化
m_txp (16.026),//送信電力(dB)を20で初期化
m_EDT (-96),
//m_txp (20),//送信電力(dB)を20で初期化
//m_EDT (-84.5),
m_lossModelName ("ns3::LogDistancePropagationLossModel"),//電波伝搬損失モデルの名前を""で初期化
m_rate ("8192bps"),//レートを"2048bps"で初期化
m_phyMode ("OfdmRate24MbpsBW10MHz"),//wifiの物理層のモードを変調方式ofdm,レート6Mbps,帯域幅10MHzで初期化
m_packetSize("1024"),
m_totalSimTime (200.0),// シミュレーション時間の合計を300.01で初期化
m_fileName("/home/hry-user/dataTemp/data.txt"),
m_nodeSpeed (7.0),//ノードの速度を20(m/s)で初期化する
m_nodePause (1.0),//ノードの停止時間を0(s)で初期化
m_areaSizeXY (600),
m_divNumXY (5),
...
m_packetLoss (0),
m_numHops(0),
m_animFile ("vanet-animation.xml")
{
  for(uint32_t i=0;i<6;i++){
    m_sourceNode[i]=i;
    m_sinkNode[i]=m_nNodes-(i+1);
  }
...
{//コマンドライン引数を処理する　argc:引数の数,argv:引数
  ...
  cmd.AddValue ("areaSize", "size of area (m)", m_areaSizeXY);
...
{
  m_adhocTxNodes.Create (m_nNodes);//指定したノード数分のノードを作成する
}
//メソッド15
void
VanetRoutingExperiment::ConfigureDevices ()//チャネルを構成する
{
  ...
  wifiChannel.AddPropagationLoss (m_lossModelName,
    ...
    //"ReferenceLoss"    ,DoubleValue(47.86043223));
  ...
  {//モビリティを設定する
    ...
    coordinate.resize(2);
    for(int i=0;i<2;i++)
    coordinate[i].resize(m_divNumXY+1);
    ...
    for(uint32_t i=0;i<36;i++){
      int elementX=i%6;//行
      int elementY=i/(m_divNumXY+1);;//列
      pos->Add(Vector(coordinate[0][elementX],coordinate[1][elementY],0));
    }

    std::stringstream ssSpeed;
    ssSpeed << "ns3::UniformRandomVariable[Min=3.0|Max=" << m_nodeSpeed << "]";
    //ssSpeedに文字列:"ns3::UniformRandomVariable[Min=0.0|Max=",数値:ノードの速度20(m/s)を追加する
    //ssSpeed << "ns3::ConstantRandomVariable[Constant=" << m_nodeSpeed << "]";
    std::stringstream ssPause;
    ssPause << "ns3::ConstantRandomVariable[Constant=" << m_nodePause << "]";
    //ssPauseに文字列:"ns3::ConstantRandomVariable[Constant=",数値:ノードの停止時間0(s)を追加する
    mobilityAdhoc.SetMobilityModel ("ns3::GridMobilityModel",//RandomWaypointMobilityModel移動モデル
    "Speed", StringValue (ssSpeed.str ()),//速度5~20(m/s)
    "Pause", StringValue (ssPause.str ()),
    "AreaSize", UintegerValue(m_areaSizeXY));//一時停止時間0(s)

    //mobilityAdhoc.SetMobilityModel ("ns3::ConstantPositonMobilityModel");//静止モデル

    mobilityAdhoc.SetPositionAllocator (pos);//すべてのノードの初期位置を割り当てるために使用する位置アロケータを設定する
    mobilityAdhoc.Install (m_adhocTxNodes);//設定した位置アロケータに従って、ノードの集合を配置する
    //  m_streamIndex += mobilityAdhoc.AssignStreams (m_adhocTxNodes, m_streamIndex);
    /*ノード上のモビリティモデルによって使用される確率変数に、固定された確率変数ストリーム番号を割り当てる
    このヘルパーによって割り当てられたストリームインデックスの数を返す。　
    m_adhocTxNodes:固定ストリームを使用するためにMobilityModelを変更する必要があるノードのセット,m_streamIndex:使用する最初のストリームインデックス*/
    //最初はすべてのノードが移動すると仮定する
    WaveBsmHelper::GetNodesMoving ().resize (m_nNodes, 1);
    //移動するノードインディケータ(移動するかどうか)のリストを取得し、動的配列の要素数をノード数とし、要素は1で初期化する
  }
  ...
  VanetRoutingExperiment::RunFlowMonitor()
  {
    ...
    //uint32_t src,dst;//送信元ノード番号,宛先ノード番号

    /*for(int j=0;j<6;j++){//セッションの数だけループする
      src=m_sourceNode[j];//送信元ノード番号
      dst=m_sinkNode[j];//宛先ノード番号


      //送信元アドレス
      Ipv4InterfaceAddress srcAddrress = m_adhocTxNodes.Get(src)->GetObject <Ipv4> ()->GetAddress(1, 0);//GetObjectメソッドでNodeポインタを参照してIpv4ポインタに集約したオブジェクトを取得
      //GetAddressメソッドでインターフェイス番号1,Ipv4InterfaceAddressのインデックス番号0に関連付けられたIpvInterfaceAddressを取得
      Ipv4Address srcIpAddress = srcAddrress.GetLocal();//GetLocalメソッドでローカルアドレスを取得する
      //宛先アドレス
      Ipv4InterfaceAddress dstAddrress = m_adhocTxNodes.Get(dst)->GetObject <Ipv4> ()->GetAddress(1, 0);
      Ipv4Address dstIpAddress = dstAddrress.GetLocal();
      //表示する
      //std::cout<<"\n";
      ...
      //m_flowMonitor->CheckForLostPackets ();//消失したと思われるパケットをすぐ確認する
