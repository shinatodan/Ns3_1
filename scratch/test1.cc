                                     ...
                                     Ipv4InterfaceContainer & ic)
{
  ...
  UdpEchoServerHelper server1 (m_port);
	ApplicationContainer apps = server1.Install (c.Get(src));
  double startTime =double(rand()%11)/10.0+1.0;
	apps.Start (Seconds (startTime));
	apps.Stop (Seconds (m_totalSimTime));

  UdpEchoClientHelper client (ic.GetAddress (src), m_port);
	client.SetAttribute ("MaxPackets", UintegerValue (maxPacketCount));
	client.SetAttribute ("Interval", TimeValue (interPacketInterval));
	client.SetAttribute ("PacketSize", UintegerValue (packetSize));

	apps = client.Install (c.Get (src));
	apps.Start (Seconds (startTime+1));
	apps.Stop (Seconds (m_totalSimTime));*/
